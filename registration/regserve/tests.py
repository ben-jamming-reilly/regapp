...
class DataTest(TestCase):
    ...
    def test_student_api(self):
        ...
        print(
            f'STUDENT API TEST: insde test, response is {students_response}\n')
        self.assertEqual(students_response.status_code, 200)
        print(
            f'STUDENT API TEST: insde test, response content is {students_response.content}\n')
        student_stream = io.BytesIO(students_response.content)
        print(
            f'STUDENT API TEST: insde test, student stream is {student_stream}\n')
        student_api_data = JSONParser().parse(stream=student_stream)
        print(
            f'STUDENT API TEST: insde test, student api data is {student_api_data}\n')
        first_student_data = student_api_data[0]
        print(
            f'STUDENT API TEST: insde test, first student data is {first_student_data} id is {first_student_data["id"]}\n')
        first_student_db = Student.objects.get(id=first_student_data['id'])
        print(
            f'STUDENT API TEST: insde test, first student db is {first_student_db}\n')
        first_student_serializer = StudentSerializer(
            first_student_db, data=first_student_data)
        print(
            f'STUDENT API TEST: insde test, first student serializer is {first_student_serializer}\n')
        print(
            f'STUDENT API TEST: insde test, first student serializer is valid? {first_student_serializer.is_valid()}\n')
        first_student_api = first_student_serializer.save()
        print(
            f'STUDENT API TEST: insde test, first student api is {first_student_api}\n')
    ...
    def test_student(self):
        ...
        # confirm that first student was retrieved
